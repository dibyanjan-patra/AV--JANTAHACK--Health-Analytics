...
health_camp <- fread(file.choose())

health_1 <- fread(file.choose())
health_2 <- fread(file.choose())
health_3 <- fread(file.choose())

health_1[, V5 := NULL]
setnames(health_1, "Health_Score", "Health_Score_1")
setnames(health_2, "Health Score", "Health_Score_2")

patient <- fread(file.choose())

train[, train_flag := 1] # adding column and assigning 1 to it
test[, train_flag := 0]  #adding column and assigning 0 to it in test dataset
...
model_xgb_cv <- xgb.cv(params=params, xgtrain, nrounds = 100, nfold = 5, early_stopping_rounds = 30, prediction = TRUE)
